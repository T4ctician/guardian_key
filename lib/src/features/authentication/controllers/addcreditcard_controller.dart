...
class CreditCardController extends GetxController {
  ...
  Future<void> addCreditCard(CreditCardModel card) async {
    try {
      await _creditCardRepo.createCreditCard(card, masterPassword);
  ...
  Future<void> updateCreditCard(CreditCardModel card) async {
    try {
      await _creditCardRepo.updateCreditCardRecord(card, masterPassword);
...
Future<CreditCardModel?> fetchCardByCardholderNameBankAndLast4(String cardholderName, String issuingBank, String last4Digits) async {
    return await _creditCardRepo.getCreditCardByCardholderNameBankAndLast4(cardholderName, issuingBank, last4Digits, masterPassword);
}
...
Future<List<CreditCardModel>> fetchCardsByCardholderNameAndBank(String cardholderName, String issuingBank) async {
    return await _creditCardRepo.getCreditCardByCardholderNameAndBank(cardholderName, issuingBank, masterPassword);
}
}
